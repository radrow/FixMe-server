...
import controllers.Evacuation.isEvacuation
import controllers.Validator.validateUser
			...
			case Some(client) =>
				...
				try {
					...
					report_to_add.tags.foreach(t =>
						                           Await.result(db.run(DBIO.seq(Tables
						                                                        .reportTags += (all_tags(t),
                                                           report_id))), Duration
						                                                                                                   .Inf)
					)
					Await
		...
		validateUser(request) match {
			case Some(client) =>
				...
				val tagP: Seq[models.Tag] => Boolean = query.get("tag").map(s =>
					                                                            (rt: Seq[models.Tag]) => rt.map(_.name)
					                                                                                     .intersect(s)
					                                                                                     .nonEmpty)
				                                       .getOrElse((_: Seq[models.Tag]) => true)
